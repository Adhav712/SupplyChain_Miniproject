...
function createOrg5 {
  ...
  set -x
	fabric-ca-client enroll -u https://peer0:peer0pw@localhost:13054 --caname ca-org5 -M "${PWD}/../organizations/peerOrganizations/org5.example.com/peers/peer0.org5.example.com/msp" --csr.hosts peer0.org5.example.com --tls.certfiles "${PWD}/fabric-ca/org5/tls-cert.pem"
  { set +x; } 2>/dev/null
